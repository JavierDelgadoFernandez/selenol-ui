...
/**
 * Store to keep the information about the groups where the user is a member.
 */
class UserMemberGroupsStore extends ReduceStore {
    constructor () {
        super(AppDispatcher);
    }
    ...
    reduce(state, action) {
        switch (action.type) {
            case UserMemberGroupsActionTypes.REFRESH:
                return action.groups.reduce(
                    (p, g) => p.set(g.group_id, g), Map());
            case UserMemberGroupsActionTypes.NEW:
...
export default new UserMemberGroupsStore();
